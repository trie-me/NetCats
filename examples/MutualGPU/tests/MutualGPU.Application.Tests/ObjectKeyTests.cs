...
{
    [Fact]
    public void Provider_object_paths_are_hmac_derived_and_queue_markers_are_tiered()
    {
        var keys = new MutualGpuObjectKeys([1, 2, 3]);
        var identity = keys.NodeIdentity("provider-secret");
        var queue = keys.QueueMarker(
            CapabilityId.New(),
            new ResourceProfile(ResourceTier.Medium, ResourceTier.Large),
            new DateTimeOffset(2026, 7, 20, 0, 0, 0, TimeSpan.Zero),
            TaskId.New());

        Assert.DoesNotContain("provider-secret", identity.Value, StringComparison.Ordinal);
        Assert.StartsWith("mutualgpu/v1/nodes/", identity.Value, StringComparison.Ordinal);
        Assert.Contains("/queue/", queue.Value, StringComparison.Ordinal);
        Assert.Contains("/04/", queue.Value, StringComparison.Ordinal);
    }
}
